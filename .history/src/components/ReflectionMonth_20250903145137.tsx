...
export function ReflectionMonth({ entries }: { entries: JournalEntry[] }) {
  ...
  // 🔹 Filter only entries from the current month and year
  ...
  if (windowed.length === 0) {
    return (
      <div className="text-sm">
        No entries in the current month yet. A two-minute check-in can start a streak.
      </div>
    );
  }
  ...
  // Determine top recurring themes
  const themes: Record<string, number> = {};
  windowed.forEach((e) =>
    (e.themes || []).forEach(
      (t) => (themes[t] = (themes[t] || 0) + 1)
    )
  );
  ...
  // Summary text
  const longText = [
    `You journaled ${windowed.length} time${windowed.length > 1 ? "s" : ""} this month with an average mood score of ${avg.toFixed(2)}.`,
  ...
  // Prepare data for the line chart, rounding to two decimals
  const data = windowed.map((e) => ({
    date: (e.date || "").slice(5),
    score: round2(e.sentiment),
  }));
