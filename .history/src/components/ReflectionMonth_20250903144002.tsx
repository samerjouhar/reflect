...
export function ReflectionMonth({ entries }: { entries: JournalEntry[] }) {
  // Prefer last 30 days so seeded demo always shows up.
  let windowed = entries.filter((e) => withinDays(e.date, 30));

  // If nothing in last 30 days (unlikely), fall back to current month.
  ...
  if (windowed.length === 0) {
    return (
      <div className="text-sm">
        No entries in the last month yet. A two-minute check-in can start a streak.
      </div>
    );
  }
  ...
  const avg =
    windowed.reduce(
      (a, e) => a + (Number.isFinite(e.sentiment) ? e.sentiment : 0),
      0
    ) / windowed.length;

  const themes: Record<string, number> = {};
  windowed.forEach((e) =>
    (e.themes || []).forEach((t) => (themes[t] = (themes[t] || 0) + 1))
  );
  ...
  const longText = [
    `You journaled ${windowed.length} time${
      windowed.length > 1 ? "s" : ""
    } in the last ~30 days with an average mood score of ${avg.toFixed(2)}.`,
  ...
  // 👇 Round scores to two decimals for the chart
  const data = windowed.map((e) => ({
    date: (e.date || "").slice(5),
    score: Number.isFinite(e.sentiment)
      ? parseFloat(e.sentiment.toFixed(2))
      : 0,
  }));
        ...
        <div className="h-48">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
              ...
              <YAxis domain={[-2, 2]} />
              <RTooltip
                formatter={(value: number) => value.toFixed(2)} // Tooltip rounded too
              />
              <Line
                type="monotone"
                dataKey="score"
